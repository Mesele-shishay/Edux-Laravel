...
class SchoolSessionRepository implements SchoolSessionRepositoryInterface
{
    ...
    public function browse($request) {
        try {
            if(session()->has('browse_session_id')
                ...
                session()->forget('browse_session_name');
            } else {
                session()->put('browse_session_id',$request['session_id']);
                session()->put('browse_session_name', $this->getSessionById($request['session_id'])->session_name);
            }
        } catch (\Exception $e) {
